...
List available genomes (BSgenome)
...
We provided a small example data set with this package. The data consists of somatic mutation catalogues of
9 normal human adult stem cells from 3 tissues (Blokzijl et al., 2016).  
...
With \Rfunction{get\_type\_context}, you can retrieve the types and contexts
...
base substitution catalogs.

The \Rfunction{plot\_spectrum} function plots the mean relative contribution
of each of the 6 base substitution types over all samples.  Error bars indicate standard
...
\begin{figure}[H]
\begin{center}
<<combine_plot_spectrum, fig=TRUE, width=9, height=2.7>>=
library("gridExtra")
...
%TODO: Roel wil je ervoor zorgen dat de plots er zo uitzien als in de markdown op github?

\begin{figure}[H]
\begin{center}
<<combine_plot_spectrum_2, fig=TRUE, width=11, height=4>>=
grid.arrange(p4, p5, ncol=2, widths=c(4,2))
@
\end{center}
\end{figure}
...
\begin{figure}[H]
\begin{center}
<<plot_96_profile, fig=TRUE, width=9, height=7>>=
...
\begin{figure}[H]
\begin{center}
<<estimate_rank, fig=TRUE, width=9, height=7>>=
...
% TODO: ik mis hier de plot van de signatures
...
\begin{figure}[H]
\begin{center}
<<plot_contribution, fig=TRUE, width=9, height=3>>=
...
% TODO: ik mis hier de plot van de contributions, check markdown op github
Plot the contribution of the signatures in absolute number of mutations:
\begin{figure}[H]
\begin{center}
<<plot_contribution_2, fig=TRUE, with=9, height=3>>=
...
% TODO: ik mis ook deze plot
Compare reconstructed mutation profile with original mutation profile:
\begin{figure}[H]
\begin{center}
<<plot_compare_profiles, fig=TRUE, width=9, height=7>>=
...
\begin{figure}[H]
\begin{center}
<<plot_compare_profiles_2, fig=TRUE, width=10, height=7>>=
...
% TODO: verhoudingen aanpassen plots, zie markdown, links is veel te klein, rechts te groot
\begin{figure}[H]
\begin{center}
<<plot_192_profile, fig=TRUE, width=9, height=5>>=
# combine plots
...
genome build hg19 from Ensembl using biomaRt.  For other datasets, see the 
\Rpackage{biomaRt} documentation (Durinck et al. 2005).

Load biomaRt package

<<install_biomaRt>>=
## To install biomaRt comment outthe following lines:
...
\begin{figure}[H]
\begin{center}
<<plot_enrichment_depletion, fig=TRUE, width=9, height=6>>=
