...
\url{https://www.biorxiv.org/content/early/2016/10/17/071761}
...
occurrences for the VCF objects in the \texttt{GRangesList}. For
...
The gridExtra package can be used to combine multiple plots:
...
Plot 96 profile of two sample in a more condensed plotting format:
\begin{figure}[H]
\begin{center}
<<plot_96_profile_3, fig=TRUE, width=7, height=3>>=
plot_96_profile(mut_mat[,c(1,7)], condensed = T)
@
\end{center}
\end{figure}
...
pch2 <- plot_contribution_heatmap(nmf_res$contribution, cluster_samples = F)
...
# Match the order of the mutation tyoes to MutationalPatterns standard
...
plot_96_profile(cancer_signatures[,1:2], condensed = T, ymax = 0.3)
@
\end{center}
\end{figure}
...
The similarity between each mutational profile and each COSMIC signature reflects how well each mutational profile can be explained by each signature individually. The samples can be hierarchically clustered. The advantage of this heatmap representation is that it shows in a glance the similarity in mutation profiles between samples, while at the same time providing information on which signatures are most prominent. 

<<cos_sim_cosmic_samples, fig=TRUE, width=7, height=3.5>>=
# Calculate pairwise cosine similarity with cosmic cancer signatures
cos_sim_samples_signatures = cos_sim_matrix(mut_mat, cancer_signatures)
# Plot heatmap with specified signature order
plot_cosine_heatmap(cos_sim_samples_signatures, sig_order = cosmic_order, cluster_samples = TRUE)
@



\subsubsection{Find optimal contribution of COSMIC signatures}
...
Plot contribution of the COSMIC signatures in each sample as a barplot.
...
Compare the reconstructed mutation profile of sample 1
with its original mutation profile:
...
than one gene body on on different strand.
...
object. Function returns ``-'' for positions outside gene bodies, and positions
that overlap with more than one gene on different strands.
...
\begin{figure}[H]
\begin{center}
<<plot_strand_bias, fig=TRUE, width=7.5, height=5>>=
...
\begin{figure}[H]
\begin{center}
<<plot_strand_bias_rep, fig=TRUE, width=7.5, height=5>>=
