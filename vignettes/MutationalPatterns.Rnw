...
<<Loading reference data, eval=FALSE>>=
library(BSgenome)
available.genomes()
@
...
Retrieve the context (1 base upstream and 1 base downstream) of the
positions in the vcf object from the reference genome:

<<get_mut_context>>=
get_mut_context(vcfs[[1]], ref_genome)
@

Retrieve the types and context of the base substitution types for all
positions in the vcf object.  For the base substitutions that are converted
to the conventional base substitution types, the reverse complement of
...
\begin{figure}
\begin{center}
<<plot_spectrum, fig=TRUE,height=3>>=
tissue = c(
    "colon", "colon", "colon",
...
\begin{figure}
\begin{center}
<<plot_spectrum, fig=TRUE>>=
my_colors = c("pink", "orange", "blue", "lightblue", "green", "red", "purple")
plot_spectrum(type_occurences, CT = TRUE, legend = TRUE, colors = my_colors)
@
\end{center}
\end{figure}
...
\begin{figure}
\begin{center}
<<plot_contribution, fig=TRUE,width=8>>=
# Provide signature names (optional)
...
<<plot_compare_profiles, fig=TRUE>>=
plot_compare_profiles(mut_mat[,1], 
...
<<plot_signature_strand_bias, fig=TRUE>>=
...
<<plot_enrichment_depletion, fig=TRUE>>=
# Test for significant enrichment or depletion in the genomic regions
# samples can be collapsed into groups, here the analysis is performed
# per tissue type
distr_test = enrichment_depletion_test(distr, by = tissue)
